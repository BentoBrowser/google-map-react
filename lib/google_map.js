...
var isFullScreen = function isFullScreen(window) {
...
var GoogleMap = (function(_Component) {
  ...
  function GoogleMap(props) {
    ...
    _this._initMap = function() {
      ...
      var maps = _this.props.maps.maps;
      ...
      var map = new maps.Map(_this.googleMapDom_, mapOptions);
      ...
      var overlay = Object.assign(new maps.OverlayView(), {
        onAdd: function onAdd() {
          ...
          var div = this_._getWindow().document.createElement('div');
      ...
      maps.event.addListener(map, 'zoom_changed', function() {
        ...
        if (this_.geoService_.getZoom() !== map.getZoom()) {
          ...
          if (mapsVersion < DRAW_CALLED_DURING_ANIMATION_VERSION) {
            ...
            if (
              new Date().getTime() - _this.zoomControlClickTime_ < TIMEOUT_ZOOM
            ) {
    ...
    _this._setViewSize = function() {
      ...
      if (isFullScreen(_this._getWindow())) {
        _this.geoService_.setViewSize(
          _this._getWindow().innerWidth,
          _this._getWindow().innerHeight
        );
      } else {
        _this.geoService_.setViewSize(
          _this.googleMapDom_.clientWidth,
          _this.googleMapDom_.clientHeight
        );
      }
  ...
  GoogleMap.prototype.componentWillUnmount = function componentWillUnmount() {
    this.mounted_ = false;
    if (this.googleMapDom_) {
      this.googleMapDom_.removeEventListener(
        'mousedown',
        this._onMapMouseDownNative,
        true
      );
    }
    this._getWindow().removeEventListener('resize', this._onWindowResize);
    this._getWindow().removeEventListener('keydown', this._onKeyDownCapture);
...
GoogleMap.propTypes = {
  ...
  maps: _propTypes2.default.object.isRequired,
